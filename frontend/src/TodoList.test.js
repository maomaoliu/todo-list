...
describe('<TodoList>', () => {
  ...
  test('should edit todo item correctly', async () => {
    ...
    fireEvent.click(screen.getByLabelText('isComplete'));

    await wait(() => expect(TodoApi.updateTodo).toHaveBeenCalled());
    expect(screen.getByLabelText('isComplete').checked).toBe(true);
  });

  test('should update todo item complete status correctly', async () => {
    jest
      .spyOn(TodoApi, 'updateTodo')
      .mockImplementation(() => Promise.resolve(updateItem));
